...
<x-layouts::storefront title="Beranda">
    @if ($demoStore)
        @php
            ...
            $journey = match ($demoStore['layout'] ?? null) {
                'simple' => [
                    ...
                    'steps' => [['01', 'Pilih menu'], ['02', 'Bayar QRIS'], ['03', 'Pantau pesanan']],
                ...
                'business' => [
                    ...
                    'description' => 'Pilih produk tanpa membuat akun, bayar lewat QRIS atau transfer bank, lalu pantau statusnya dari link pesanan.',
                    'steps' => [['01', 'Pilih produk'], ['02', 'QRIS / transfer'], ['03', 'Pantau pesanan']],
                    'image_alt' => 'Pilihan perangkat kerja Shicomp Store',
                ],
                default => [
                    'eyebrow' => 'Belanja mulus',
                    'title' => 'Dari menemukan sampai barang tiba, prosesnya ringkas.',
                    'description' => 'Belanja tanpa membuat akun, pilihan pembayaran lengkap, dan link pribadi untuk memantau pesanan.',
                    'steps' => [['01', 'Pilih barang'], ['02', 'Banyak pilihan bayar'], ['03', 'Pantau pesanan']],
                    'image_alt' => 'Koleksi interior pilihan Nara Atelier',
                ],
            };
            $journeyImage = $demoStore['story_image'] ?? $demoStore['hero_banner'] ?? null;
            $bestSellerVisual = $bestSeller ? \App\Support\StorefrontContext::productVisual($bestSeller) : null;
            $bestSellerDialog = $bestSeller ? [
                'name' => $bestSeller->name,
                'price' => (float) $bestSeller->price,
                'stock' => $bestSeller->stock,
                'icon' => $bestSellerVisual['icon'] ?? mb_substr($bestSeller->name, 0, 1),
                'color' => $bestSellerVisual['color'] ?? '#e3f4f3',
                'cartUrl' => \App\Support\StorefrontContext::route('cart.store', ['product' => $bestSeller]),
                'buyUrl' => \App\Support\StorefrontContext::route('cart.buy', ['product' => $bestSeller]),
            ] : null;
        @endphp
        @if (($demoStore['layout'] ?? null) === 'simple')
            <section class="relative overflow-hidden bg-[#fffaf1] py-16 sm:py-24">
                ...
                <div class="relative mx-auto grid max-w-6xl items-center gap-10 px-5 lg:grid-cols-[1fr_.75fr] sm:px-8">
                    <div><span class="inline-flex rounded-full bg-tosca-tint px-4 py-2 text-xs font-semibold tracking-wide text-tosca uppercase">Kopi enak, tanpa ribet</span><h1 class="mt-6 max-w-3xl text-4xl leading-[1.08] text-navy sm:text-6xl">{{ $demoStore['headline'] }}</h1><p class="mt-5 max-w-xl text-base leading-7 text-ink-soft sm:text-lg">{{ $demoStore['description'] }}</p><div class="mt-8"><x-ui.button :href="\App\Support\StorefrontContext::route('products.index')" variant="navy">Lihat menu</x-ui.button></div><div class="mt-9 flex flex-wrap gap-x-6 gap-y-2 text-sm text-ink-soft"><span>✓ Pesan tanpa akun</span><span>✓ Bayar via QRIS</span><span>✓ Link status pesanan</span></div></div>
                    @if ($bestSeller)<article class="rounded-[2rem] border border-tosca/20 bg-white p-5 shadow-card"><div class="flex aspect-square items-center justify-center rounded-[1.5rem] bg-tosca-tint text-8xl text-navy/20">{{ \Illuminate\Support\Str::substr($bestSeller->name, 0, 1) }}</div><div class="p-3 pt-5"><p class="text-xs font-semibold tracking-wide text-tosca uppercase">Menu paling disukai</p><h2 class="mt-2 text-2xl text-navy">{{ $bestSeller->name }}</h2><div class="mt-4 flex items-center justify-between"><strong class="text-lg text-navy">Rp{{ number_format((float) $bestSeller->price, 0, ',', '.') }}</strong><span x-data data-product="{{ json_encode($bestSellerDialog) }}"><x-ui.button type="button" variant="navy" class="cursor-pointer px-4 py-2.5" x-on:click="$dispatch('open-quantity-dialog', { intent: 'cart', product: JSON.parse($root.dataset.product) })">Pesan</x-ui.button></span></div></div></article>@endif
                </div>
            </section>
        ...
        @else
            <section class="relative min-h-[42rem] overflow-hidden bg-[#211a17] text-white">
                ...
                <div class="relative mx-auto flex min-h-[42rem] max-w-7xl items-center px-5 py-20 sm:px-8"><div class="max-w-2xl"><p class="text-xs tracking-[0.35em] text-[#e9c6b7] uppercase">Nara Atelier · Koleksi terbaru</p><h1 class="mt-7 text-5xl leading-[1.02] font-normal text-white sm:text-7xl">{{ $demoStore['headline'] }}</h1><p class="mt-7 max-w-xl text-lg leading-8 text-white/70">{{ $demoStore['description'] }}</p><div class="mt-10 flex flex-wrap gap-4"><a href="{{ \App\Support\StorefrontContext::route('products.index') }}" class="inline-flex items-center justify-center rounded-none bg-[#f3e8df] px-7 py-3.5 text-sm font-semibold text-[#211a17] transition hover:bg-white">Jelajahi koleksi</a>@if($bestSeller)<a href="{{ \App\Support\StorefrontContext::route('products.show', ['product' => $bestSeller]) }}" class="inline-flex items-center border-b border-white/50 px-1 py-3 text-sm text-white transition hover:border-white">Lihat karya pilihan →</a>@endif</div><div class="mt-14 flex flex-wrap gap-7 text-xs tracking-wide text-white/55 uppercase"><span>Koleksi terkurasi</span><span>Ongkir terpantau</span><span>Dukungan prioritas</span></div></div></div>
            </section>
        @endif
    @else
        <section class="relative overflow-hidden bg-linear-to-br from-navy via-navy-mid to-navy py-20 text-white sm:py-28">
    ...
    @if ($demoStore && $tierLevel >= 2)
        @php
            $shoppingHighlights = [
                ['Cari cepat', 'Temukan barang lewat kata kunci tanpa menelusuri seluruh katalog.'],
                ['Saring harga & stok', 'Sesuaikan pilihan dengan anggaran dan ketersediaan barang.'],
                ['Rekomendasi terkait', 'Lihat pilihan lain yang serupa dari kategori yang sedang dibuka.'],
                ['Pesan tanpa akun', 'Langsung checkout, lalu pantau pesanan lewat link pribadi.'],
            ];
        @endphp
        <section class="border-y border-line bg-white">
            <div class="mx-auto max-w-7xl px-5 py-16 sm:px-8">
                <div class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                    <div>
                        <p class="text-xs font-semibold tracking-[0.2em] text-tosca uppercase">Belanja lebih mudah</p>
                        <h2 class="mt-3 text-3xl">Katalog yang enak dijelajahi</h2>
                        <p class="mt-3 text-sm text-ink-soft">Cari, saring, dan bandingkan pilihan sebelum memutuskan.</p>
                    </div>
                    <a href="{{ \App\Support\StorefrontContext::route('products.index') }}" class="text-sm font-semibold text-navy">Jelajahi seluruh katalog →</a>
                </div>
                <div class="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($shoppingHighlights as [$title, $copy])
    ...
    @if ($demoStore && $tierLevel >= 3)
        <section class="border-y border-line bg-white">
            <div class="mx-auto max-w-7xl px-5 py-20 sm:px-8">
                <div class="max-w-2xl">
                    <p class="text-xs tracking-[0.3em] text-tosca uppercase">Pengalaman Nara</p>
                    <h2 class="mt-5 text-4xl sm:text-5xl">Lebih dari sekadar barang. Sebuah layanan yang dipikirkan.</h2>
                </div>
                <div class="mt-12 grid gap-px overflow-hidden border border-line bg-line md:grid-cols-3">
                    @foreach ([['01', 'Kurasi personal', 'Konsultasi untuk menyusun pilihan yang menyatu dengan ruangmu.'], ['02', 'Pengiriman terpantau', 'Estimasi tarif dan layanan kurir yang terhubung langsung.'], ['03', 'Layanan purna beli', 'Dukungan hingga akhir pekan untuk pengalaman yang tenang.']] as [$number, $title, $copy])
    ...
    <section class="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24">
        ...
        @elseif (($demoStore['layout'] ?? null) === 'editorial')
            <x-ui.section-header eyebrow="Pilihan terkurasi" title="Karya yang punya kehadiran dan tujuan" description="Material, proporsi, dan detail yang dipilih untuk ruang yang lebih personal." />
        @else
    ...
    @if ($demoStore)
        @php
            $faqItems = [
                ['Apakah harus membuat akun?', 'Tidak. Pilih barang, isi data penerima, lalu simpan link status pesanan yang bisa dikirim ke WhatsApp.'],
                ['Apakah pesanan bisa dipantau?', 'Bisa. Setiap pesanan punya link pribadi untuk melihat status pembayaran dan prosesnya.'],
            ];

            if ($tierLevel >= 2) {
                $faqItems[] = ['Apakah barang bisa dicari dan disaring?', 'Bisa. Gunakan kolom pencarian, kategori, serta saringan harga dan stok di halaman katalog.'];
            }

            if ($tierLevel >= 3) {
                $faqItems[] = ['Apakah saya bisa menyimpan pilihan?', 'Bisa. Simpan barang ke daftar favorit dan buka lagi kapan saja selama sesi belanja.'];
            }

            $faqItems[] = ['Bagaimana cara membayar?', 'Pembayaran diproses otomatis melalui Midtrans. Pilihan metodenya muncul setelah pesanan dibuat.'];
        @endphp
