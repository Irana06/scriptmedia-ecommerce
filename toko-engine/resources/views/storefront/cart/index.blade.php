<x-layouts::storefront title="Keranjang">
    <section class="mx-auto max-w-7xl px-5 py-12 sm:px-8 sm:py-18">
        ...
        @if ($items->isEmpty())
            <x-ui.empty-state class="mt-8" icon="cart" title="Keranjangmu masih kosong" description="Temukan produk favorit, lalu tambahkan ke keranjang untuk melanjutkan pesanan." action-label="Mulai belanja" :action-href="route('products.index')" />
        @else
                ...
                <div class="space-y-4">
                    @foreach ($items as $item)
                        <x-ui.card class="flex flex-col gap-5 sm:flex-row sm:items-center">
                            ...
                            <form method="POST" action="{{ route('cart.update', $item['product']) }}" class="flex items-center gap-2">@csrf @method('PATCH')<input type="number" name="quantity" value="{{ $item['quantity'] }}" min="0" max="{{ $item['product']->stock }}" class="w-20 rounded-xl border border-line px-3 py-2"><x-ui.loading-button loading-label="Memperbarui..." variant="navy" class="px-4 py-2 cursor-pointer">Update</x-ui.loading-button></form>
                            <div class="font-semibold text-navy">Rp{{ number_format($item['line_total'], 0, ',', '.') }}</div>
                            <form method="POST" action="{{ route('cart.destroy', $item['product']) }}">@csrf @method('DELETE')<button class="text-sm text-red-600 cursor-pointer">Hapus</button></form>
                        </x-ui.card>
                    @endforeach
                </div>
                <x-ui.card class="h-fit lg:sticky lg:top-24"><h2 class="text-xl text-navy">Ringkasan</h2><div class="mt-5 flex justify-between border-t border-line pt-5 text-sm text-ink-soft"><span>Subtotal</span><span class="font-semibold text-navy">Rp{{ number_format($subtotal, 0, ',', '.') }}</span></div><p class="mt-3 text-xs leading-5 text-ink-soft">Biaya pengiriman akan dikonfirmasi oleh toko sesuai alamat tujuan.</p><div class="mt-6"><x-ui.button :href="route('checkout.create')" class="w-full">Lanjut checkout</x-ui.button></div></x-ui.card>
            </div>
        @endif
    </section>
