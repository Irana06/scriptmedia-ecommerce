        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_300px]">
            <x-ui.card>
                <form method="POST" action="{{ route('onboarding.store', $plan) }}" class="space-y-6">
                    ...
                    <div><label for="desired_subdomain" class="text-sm font-semibold">Subdomain yang diinginkan</label><div class="mt-2 flex rounded-xl border border-line bg-white focus-within:ring-2 focus-within:ring-tosca"><input id="desired_subdomain" name="desired_subdomain" value="{{ old('desired_subdomain') }}" required class="min-w-0 flex-1 rounded-l-xl px-4 py-3 text-navy outline-none" placeholder="tokosenja"><span class="flex items-center border-l border-line px-4 text-sm text-ink-soft">.{{ config('tenancy.base_domain') }}</span></div></div>
