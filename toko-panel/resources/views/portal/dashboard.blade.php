        ...
        <section>
            ...
            <div class="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                @forelse ($orders as $order)
                    <x-ui.card class="flex h-full flex-col"><div class="flex items-start justify-between gap-4"><div><p class="text-xs tracking-[0.16em] text-ink-soft uppercase">{{ $order->number }}</p><h3 class="mt-2 text-xl text-navy">{{ $order->business_name }}</h3></div><x-ui.badge :variant="$order->status === 'ready' ? 'tosca' : 'orange'">{{ \App\Support\StatusLabel::for($order->status) }}</x-ui.badge></div><p class="mt-4 text-sm text-ink-soft">{{ str($order->plan->name)->title() }} · Rp{{ number_format((float) $order->amount, 0, ',', '.') }}</p><x-ui.button :href="route('portal.orders.show', $order)" variant="navy" class="mt-6">Lihat detail order</x-ui.button></x-ui.card>
                @empty
                ...
                <x-ui.card class="flex h-full flex-col">
                    ...
                    <dl class="mt-6 grid grid-cols-2 gap-4 border-y border-line py-5 text-sm">
                        <div>
                            ...
                            <dd class="mt-1 font-semibold text-navy">{{ $tenant->currentSubscription?->plan?->name ? str($tenant->currentSubscription->plan->name)->title() : 'Belum ada' }}</dd>
                        </div>
                        <div>
